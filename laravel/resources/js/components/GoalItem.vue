    ...
    <div>
        <div class="row align-items-center">
            <div class="goal__check-container">
                <!-- GOAL COMPLETED ICON -->
                <i @click="goalCompletionToggle(goal)" class="goal__check zmdi zmdi-circle" v-show="!(goal.is_completed)"></i>
                <i @click="goalCompletionToggle(goal)" class="goal__check zmdi zmdi-check-circle" v-show="goal.is_completed"></i>
                <!-- END GOAL COMPLETED ICON -->
            </div>
            <div class="col">
                <!-- GOAL TEXT -->
                <p class="goal__text" contenteditable="true"
                    @keydown.enter.prevent="addGoalFromGoal(goalIndex, $event, 'spiritual')"
                    @keydown.delete="deleteGoal(goal, goalIndex, $event, 'spiritual')"
                    @keydown.tab.prevent="addTaskFromGoal(goal, $event, 'spiritual')"
                    @input="goalInput(goal, $event)"
                    v-mediumjsGoal>{{ goal.goal }}</p>
                <!-- END GOAL TEXT -->
            </div>
            <div>
                <!-- GOAL DATEPICKER -->
                <div class="row align-items-center">
                    <div>
                        <i class="goal-datepicker__icon zmdi zmdi-calendar"></i>
                    </div>
                    <div class="col">
                        <flat-pickr
                            class="form-control"
            ...
            <div>
                <!-- SHOW TASKS BUTTON -->
                <!-- <a class="btn btn-dark btn-task" data-toggle="collapse" :href="'#' + goal.vueUniqueItemId" v-show="goal.tasks.length != 0" aria-expanded="false" role="button">Tasks <i class="zmdi zmdi-chevron-down"></i></a> -->
                <a class="btn btn-dark btn-task" data-toggle="collapse" :href="'#' + goal.vueUniqueItemId" v-show="goal.tasks != 0" aria-expanded="false" role="button">Tasks <i class="zmdi zmdi-chevron-down"></i></a>
                <!-- <a class="btn btn-dark btn-task" @click="addTaskFromGoal(goal, $event, 'spiritual')" v-show="goal.tasks.length == 0" aria-expanded="false" role="button">Add Tasks</a> -->
                <a class="btn btn-dark btn-task" @click="addTaskFromGoal(goal, $event, 'spiritual')" v-show="goal.tasks == 0" aria-expanded="false" role="button">Add Tasks</a>
            </div>
        </div>
        <!-- TASKS CONTAINER -->
        <div class="tasks__container collapse" :id="goal.vueUniqueItemId">
            <div class="card card-body">
                <!-- TASK -->
                <div :class="setTaskClasses(task, taskIndex)" v-for="(task, taskIndex) in goal.tasks" :key="task.vueUniqueItemId">
                    <div class="row align-items-center">
                        <div class="task__check-container">
                            <!-- TASK COMPLETED ICON -->
                            <i @click="taskCompletionToggle(goal, task)" class="task__check zmdi zmdi-square-o" v-show="!(task.is_completed)"></i>
                            <i @click="taskCompletionToggle(goal, task)" class="task__check zmdi zmdi-check-square" v-show="task.is_completed"></i>
                            <!-- END TASK COMPLETED ICON -->
                        </div>
                        <div class="col">
                            <!-- TASK TEXT -->
                            <p class="task__text" contenteditable="true"
                                @keydown.enter.prevent="addTask(goal, $event, taskIndex)"
                                @keydown.delete="deleteTask(goal, task, taskIndex, $event)"
                                @input="taskInput(goal, task, $event)"
                                v-mediumjsTask>{{ task.task }}</p>
                            <!-- END TASK TEXT -->
                        </div>
                        <div>
                            <!-- TASK DATEPICKER -->
                            <div class="row align-items-center">
                                <div>
                                    <i class="task-datepicker__icon zmdi zmdi-calendar"></i>
                                </div>
                                <div class="col">
                                    <flat-pickr
                                        class="form-control"
